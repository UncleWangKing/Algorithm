...
public class LeetCode_42_TrappingRainWater {
    ...
    //-------------------夭折的思路分割线------------------
    //看看人家！！！
    //核心思路 找出每个点左右的最高坑位 自身和较矮的坑的差值 就是自己位置上的水量
    ...
    public static int trap(int[] height) {
        ...
        //如果还大于i当前值 差值将值放入res --- 雨水的量就是坑中每个值和坑边较短值的差之和
        for (int i = n - 1; i >= 0; --i) {
    ...
    //这个写法就是不用两边极值都拿到了再求自身水量 而是较小一个的极值拿到就可以求水量了
    //较大的极值不用确定是哪一个 只要知道肯定比较小那一侧极值大即可
